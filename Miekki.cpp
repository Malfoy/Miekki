...
IntegerCODEC& codec = *CODECFactory::getFromName("fastpfor");
...
template <typename T>
void Miekki::minhash_sketch_partition_solid_kmers(const string& reference, uint32_t& active_minimizer, T out) {
	pair<vector<minimizer>, vector<uint64_t> > sketch(minhash_sketch_partition(reference, active_minimizer));
	for (uint i(0); i < sketch.first.size(); ++i) {
		if (not check_bloom(sketch.second[i])) {
			cout<<"WTF"<<endl;
...
void Miekki::add_index(minimizer m, uint32_t partition) {
	if (number_bit_minimizer == 16) {
		cout << "jai dis UN byte" << endl;
		exit(0);
	} else if (number_bit_minimizer == 8) {
		index[partition].cells[m].push_back(file_names.size());
	} else {
...
void Miekki::get_genomes(uint32_t partition, uint32_t fingerprint, vector<uint32_t>& v) {
	...
	if(compression){
		...
		v=index[partition].cells[fingerprint];
	}
}
...
matrix::vector<score_t> Miekki::query_sequence(const string& str, uint32_t& active_minimizer) {
	...
	cout<<double_sketch.first.size()<<endl;
	active_minimizer = 0;
...
matrix::matrix<score_t> Miekki::query_sequences(vector<pair<string, uint32_t> >& batch) {
	...
	// cout<<index_size<<"index size"<<endl;cin.get();
	vector<uint32_t> genomes;
	//FOREACH MINIMIZER
	for (idx_t i_mini = 0; i_mini < idx_t(number_minimizer); ++i_mini) {
		cout<<number_minimizer<<endl;
		genomes.clear();
		for (idx_t i_batch = 0; i_batch < idx_t(batch.size()); ++i_batch) {
			...
			cout<<"MINI"<<(int)mini<<endl;
			if (mini == maximal_minimizer) {
				cout<<"W8 WAT"<<endl;
				cin.get();
				continue;
			}
			get_genomes(i_mini, mini, genomes);
			cout<<"printgenome"<<endl;
			print_vector(genomes);
			for (uint32_t j(0); j < genomes.size(); ++j) {
				if(genomes.size()!=5){
				...
				// for (idx_t genome_id = 0; genome_id < genomes.size(); ++genome_id) {
				++result(i_batch, genomes[j]);
				cout<<i_mini<<" "<<genomes[j]<<" "<<result(i_batch, genomes[j])<<endl;
					// cout<<result(i_batch, genomes[genome_id])<<endl;cin.get();
				// }
			}
		...
		// if(result(0, 0)!=i_mini){
		// 	cout<<i_mini<<" "<<result(0, 0)<<endl;
		// 	cout<<"COMPREND PAS"<<endl;
		// 	cin.get();
		// }
	}
	return result;
...
void Miekki::filter_results(matrix::refvec<score_t> results, size_t nresults, score_t min_score, double min_intersection, std::vector<similarity_score>& min_heap) {
	...
	for (idx_t genome_id = 0; genome_id < results.size(); genome_id++) {
		...
		if (min_heap.size() >= nresults) {
			if (min_heap.front().intersection > intersection)
				continue; // New element is less than the minimum
...
#pragma omp parallel num_threads(core_number)
	{
		...
		while (not in->eof()) {
			string ref, head;
			vector<pair<string, uint32_t> > batchquery;
#pragma omp critical(readfile)
			{
				getline(*in, head);
				getline(*in, ref);
			...
			batchquery.push_back({ref,0});
			uint active_minimizer(0);
			auto result = filter_results(query_sequences(batchquery), 10, 10, 0.5*threshold);
			for (auto& sim : result[0]) {
